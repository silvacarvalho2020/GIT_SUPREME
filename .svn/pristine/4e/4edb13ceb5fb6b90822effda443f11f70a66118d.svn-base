...
begin // Pega o numero do NFCe ou NFe
  ...
  begin
    ...
    begin
      ...
      //validação:
      {
        se o numero atual for menor que o numero inicial, por motivo de erro mesmo de digitação
...
begin // preço dos produtos por codigo de barras ou descricao
...
begin //Grvando NF-e
  ...
  QRY_NFeIDE_TPAMB.AsString       := ''; //  taHomologacao;  //Lembre-se de trocar esta variável quando for para ambiente de produção
  QRY_NFeIDE_VERPROC.AsString     := '1.0.0.0'; //Versão do seu sistema
...
//  NotaF.NFe.Entrega.UF      := '';
  ...
  begin
    ...
    //Dados do Produto
    QRY_NFeItensPROD_NITEM.AsInteger    := ADtaSetNFeItens.FieldByName('Prod_nItem').AsInteger;            // Já manda tudo pro banco pronto.
    ...
    QRY_NFeItensPROD_VPROD.AsFloat      := ADtaSetNFeItens.FieldByName('Prod_vProd').AsFloat;  //Levar em consideração o valor do desconto.                                                                                                     QRY_NFeItensPROD_QCOM.AsFloat * QRY_NFeItensPROD_VUNCOM.AsFloat; //deverá subtrair os descontos
...
//      EnumeradoToStr(t, ['','1', '2', '3'], [crtRegimeNormal,crtSimplesNacional, crtSimplesExcessoReceita, crtRegimeNormal]);
  ...
  ValorNFe := ValorProdutos; // Aqui deverá somar mais coisas que compoe o valor da NF-e
...
//  NotaF.NFe.infIntermed.idCadIntTran := '';
  ...
  QRY_Aux.Params[0].AsInteger := FNumFiscal; // Número Atual já incrementado
...
begin //Grvando NFC-e
  ...
  begin
    ...
    // Dados do Produto
    QRY_NFCeItensPROD_NITEM.AsInteger     := ADtaSetNFCeItens.FieldByName('Prod_nItem').AsInteger;            // Já manda tudo pro banco pronto.
    ...
    QRY_NFCeItensPROD_VPROD.AsFloat       := ADtaSetNFCeItens.FieldByName('Prod_vProd').AsFloat;  //Levar em consideração o valor do desconto.
...
//    QRY_NFCeItensICMS_VFCPST.AsFloat          := ADtaSetNFCeItens.FieldByName('ICMS_vFCPST').AsFloat;
  ...
  ValorNFCe := ValorProdutos; // Aqui deverá somar mais coisas que compoe o valor da NFC-e
